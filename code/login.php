...
<select name='username1' id='username1' class='username form-control w-50'>
<?php
foreach ($Users->getUsers(1, $Error) as $user)
	{
...
<select name='username2' id='username2' class='username form-control w-50'>
<?php
foreach ($Users->getUsers(2, $Error) as $user)
	{
	echo "<option value='" . $user['username'] . "'>" . $user['username'] . "</option>\n";
	}
?>
...
<h1>Beheren</h1>
<label for='username1'>Kies je naam:</label>
<select name='username3' id='username3' class='username form-control w-50'>
<?php
$result = $Db->query('SELECT * FROM users WHERE perm_level>=3 SORT BY username ASC');
foreach ($Users->getUsers(3, $Error) as $user)
	{
	echo "<option value='" . $user['username'] . "'>" . $user['username'] . "</option>\n";
	}
?>
