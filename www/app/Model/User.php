...
class User
{
    ...
    public static function getLiveDrafters(int $article_id): array
    {
        ...
        return User::getAllByQuery(
            'SELECT * FROM users WHERE id IN (SELECT DISTINCT(user) FROM `article_updates` WHERE article_id = '.$article_id.' AND ( update_type = '.$update_type1.') AND timestamp >= DATE_SUB(NOW(), INTERVAL 20 SECOND))'
        );
    }
